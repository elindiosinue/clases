...
void pedir_estado_civil(estado *estado) {
    int aux;
...
nodoCliente *nombre_y_apellidos_iguales(nodoCliente *lista, char nombre[], char apellidos[]) {
    while (lista != NULL) {
        if ((strcmp(nombre, lista->datosCliente.nombre) == 0) &&
            (strcmp(apellidos, lista->datosCliente.apellidos) == 0))
            return lista;
        else
            lista = lista->sig;
    }
...
void tratar_opcion(nodoCliente **lista, int op) {
    switch (op) {
        case 1:
            alta_cliente(lista);
            break;
        case 2:
            listado_completo_clientes(*lista);        // mando el contenido de la lista
            break;
        case 3:
            listado_casados_treinta(*lista);
            break;
        case 4:
            listado_nombre_apellidos(*lista);
    }
}
...
int main() {
    ...
    leer_fichero_existente(&lista,
                           nombre_f);        // Para saber los datos que hay en el fichero existente y meterlos en mi lista
